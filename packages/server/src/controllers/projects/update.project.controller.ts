...
import { updateProjectService } from "src/services/projects/update.project.service.js"
...
export const updateProjectController = async (req: Request, res: Response<ProjectResponseSuccessData | ProjectsResponseError>): Promise<Response<ProjectResponseSuccessData | ProjectsResponseError>> => {
    ...
    try {
        ...
        const {projectUpdated} = await updateProjectService({userId: userId, projectById: projectById, projectUpdate: projectUpdate })
        return res.status(200).json({message: 'User project updated successfully', projects: projectUpdated})
